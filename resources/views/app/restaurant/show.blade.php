    ...
    <div class="p-5 space-y-5">
        <div class="flex overflow-auto rounded border mx-auto max-w-full" style="width: fit-content;">
            @foreach($dates as $date)
                <a href="?date={{ $date->format("Y-m-d") }}&covers={{ $covers }}&section={{ $section }}" class="booking_date_choice py-3 px-5 text-center cursor-pointer hover:bg-red-700 hover:text-white transition duration-500 ease-in-out {{ $date->isSameDay($selectedDate) ? 'bg-red-800 text-white shadow-inner' : '' }} {{ $loop->last ? '' : 'border-r' }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="flex items-center justify-center">
                <a href="?date={{ $selectedDate }}&covers={{ $covers > 1 ? $covers - 1 : $covers }}&section={{ $section }}">
                ...
                <a href="?date={{ $selectedDate }}&covers={{ $covers + 1 }}&section={{ $section }}">
            ...
            <div class="flex items-center justify-center">
                ...
                <x-select id="location_picker">
                    @foreach($restaurant->sections()->whereHas("tables", function($query){return $query->where("bookable", 1);})->get() as $location)
                        <option value="{{ $location->id }}" {{ $section == $location->id ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
        ...
        @if($covers > $restaurant->max_booking_size($section))
            <div class="lg:wd-1/2 space-y-5">
                <p>{{ $restaurant }} can only take bookings of up to {{ $restaurant->max_booking_size($section) }} guests online in the {{ \App\Models\RestaurantSection::find($section) }}. Please contact the restaurant directly or go to <a class="external_link text-red-800 hover:text-red-700" href="{{ config("app.url") }}/restaurant/{{ $restaurant->id }}">myTables</a>.</p>
            </div>
        @else
            <div class="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                @if($services->count())
                    @foreach($services as $serviceTime)
                        <a href="/app/restaurant/{{ $restaurant->id }}/book?time={{ urlencode($serviceTime->time) }}&covers={{ $covers }}&section={{ $section }}" class="bg-white shadow rounded p-5 text-center hover:bg-red-800 hover:text-white transition ease-in-out duration-500 cursor-pointer">
    ...
    @push("scripts")
        <script>
            ...
            locationpicker.addEventListener("change", (e) => {
                window.location.replace("?date={{ $selectedDate }}&covers={{$covers }}&section=" + e.target.value)
            })
