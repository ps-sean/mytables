<x-jet-form-section submit="submit">
    ...
    <x-slot name="form">
        <p class="col-span-6">
            Tables are
            <x-select wire:model.live="restaurant.table_confirmation">
        ...
        <p class="col-span-6">
            Show dates within the next <x-jet-input wire:model.live="restaurant.show_days" type="number" min="1" max="365"/> days.
        </p>
        <p class="col-span-6">
            Show customers tables every <x-jet-input wire:model.live="restaurant.interval" type="number" min="5" max="90"/> minutes.
        </p>
        @error("restaurant.table_confirmation")<p class="col-span-6 text-red-600">{{ $message }}</p>@enderror
        <p class="col-span-6">
            Accept a maximum of
            <x-jet-input wire:model.live="restaurant.booking_timeframe.covers" type="number" min="0"/>
            covers every
            <x-jet-input wire:model.live="restaurant.booking_timeframe.minutes" type="number" min="0" max="90"/>
            minutes.
        </p>
        ...
        <p class="col-span-6">Leave a minimum of <x-jet-input wire:model.live="restaurant.turnaround_time" type="number" min="0" max="90"/> minutes between bookings. (Time to clean table before next booking)</p>
        @error("restaurant.turnaround_time")<p class="col-span-6 text-red-600">{{ $message }}</p>@enderror

        <p class="col-span-6">Pre-authorise an amount of &pound;<x-jet-input wire:model.live="restaurant.no_show_fee" type="number" min="0" max="100" step="5.00"/> per booking which can be charged in the event of a no-show.</p>
